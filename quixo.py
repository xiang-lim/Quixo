...
def check_move(board, turn, index, push_from):
    ...
    if index not in perimeter_index or board[index] not in [turn, 0]: 
        ...
        if index in corner_index: 
            ...
            # move is not avail for that specific corner cube
            if push_from not in corner_moves_dict[index]:
                return False
            else: # move is avail for that specific corner cube
                return True
            ...
            # Case 2: less than 3rd element
                    ...
                    #   B: else last col
                    #       >> T, B, L
            elif index < corner_index[2] and \
                (n % index == 0 and push_from in ["T", "B", "R"]) or (n % index != 0 and push_from not in ["T", "B", "L"]):
                    return True
            # Case 3: less than 4th aka largest element >> T,L,R
            elif index < corner_index[3] and push_from  in ["T", "L", "R"]: 
                return True
            else:
                return False
